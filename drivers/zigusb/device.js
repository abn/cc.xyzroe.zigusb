...
class ZigUSBDevice extends ZigBeeDevice {
    async onNodeInit({ zclNode }) {
        this.debug(`Node ${zclNode.nodeId} has been initialized`);
        await this.registerOnOff().catch(this.error);
        await this.registerMeasureTemperature().catch(this.error);
        await this.registerPowerMetering().catch(this.error);
        await this.registerUptime().catch(this.error);
    }
    ...
    async configureAnalogInputValueListener(endpointId) {
        ...
        if (!(storeKey in this.getStoreKeys())) {
            ...
            this.zclNode.endpoints[endpointId].clusters.analogInput.on("attr.presentValue", (value) => {
                this.debug(
                    `Present value was updated for analog input cluster (endpoint: ${endpointId}, value: ${value})`,
                );
        ...
        this.registerCapability(capabilityId, CLUSTER.ANALOG_INPUT, {
            ...
            reportParser: (value) => {
                ...
                if (value && value.startsWith(`${descriptionUnit},`)) {
                    ...
                    const multipliedValue = Math.max(0.0, presentValue) * multiplier;
